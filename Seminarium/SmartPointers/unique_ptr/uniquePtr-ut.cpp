...
protected:
    scr::unique_ptr<double> testUniquePtr_;
};
...
TEST(UniquePtrTestArrowOperator, ArrowOperatorShouldGuaranteeAcessToStructuresAndUnions) {
    scr::unique_ptr<UniquePtrArrowOperatorTest> testUniquePtr_(new UniquePtrArrowOperatorTest{});
...
TEST_F(UniquePtrTest, UniquePtrShouldMovingOwnershipToAnotherUnqiuePtr) {
    scr::unique_ptr<double> testNewPtr = std::move(testUniquePtr_);
