...
struct SharedPtrTest : ::testing::Test {
    ...
    scr::SharedPtr<int> intValuePtr;
    scr::SharedPtr<double> doubleValuePtr;
};
...
TEST(Point2D, shouldUseArrowOperator) {
    scr::SharedPtr<Point2D> ptr(new Point2D{SharedPtrTestConst::point_x, SharedPtrTestConst::point_y});
...
TEST(Point2D, shouldCreateSharedPtrWithMakeShared) {
    scr::SharedPtr<Point2D> sharedPtr = scr::make_shared<Point2D>(SharedPtrTestConst::point_x, SharedPtrTestConst::point_y);
...
TEST_F(SharedPtrTest, shouldCreateSharedPtrFromWeakPtr) {
    scr::WeakPtr<int> weakPtr{intValuePtr};

    scr::SharedPtr<int> sharedPtr{weakPtr};
